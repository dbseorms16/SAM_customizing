...
def main(args: argparse.Namespace, c_args) -> None:
    ...
    for t in targets:
        ...
        if inputs['craft']:
            ...
            for index, input_p in enumerate(input_point):
                new_input_point = np.array([input_p])
                ...
                show_points(new_input_point, new_input_label, plt.gca())

                plt.title(f"Mask {1}, IOU: {iou:.3f}, F-score: {f_score:.3f}, precision: {precision:.3f}, recall: {recall:.3f}, Confidence: {score:.3f}", fontsize=12)
                plt.axis('off')
                n_filename = f"{base}_{index}.png"
                plt.savefig(os.path.join(save_base, n_filename), bbox_inches='tight', pad_inches=0)
                plt.close()
